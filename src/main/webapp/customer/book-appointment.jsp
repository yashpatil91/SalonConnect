...
<head>
    ...
    <title>Book Appointment - SalonConnect</title>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <a href="salon-details.jsp?id=<%= salonId %>" class="text-primary hover:underline flex items-center">
            <span class="material-icons mr-1">arrow_back</span> Back to Salon Details
        </a>
    </div>

    <div class="bg-surface-light dark:bg-surface-dark rounded-3xl shadow-xl p-8 border border-gray-100 dark:border-gray-800">
        <h1 class="text-3xl font-display font-bold text-gray-900 dark:text-white mb-6">Book Your Appointment</h1>

        <% if ("slot_taken".equals(request.getParameter("error"))) { %>
        ...
        <form action="<%= request.getContextPath() %>/bookAppointment" method="POST" class="space-y-6">
            <input type="hidden" name="salonId" value="<%= salonId %>"/>

            <div>
                <label for="serviceId" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select Service *</label>
            ...
            <div>
                <label for="therapistId" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select Therapist *</label>
                <select id="therapistId" name="therapistId" required
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
                    <option value="">Choose a therapist</option>
                    <%
                        try {
                            con = DBConnection.getConnection();
                            ps = con.prepareStatement("SELECT * FROM therapists WHERE salon_id = ? ORDER BY name");
                            ps.setInt(1, salonId);
                            rs = ps.executeQuery();

                            while (rs.next()) {
                                int therapistId = rs.getInt("id");
                                String therapistName = rs.getString("name");
                                String specialization = rs.getString("specialization");
                                double rating = rs.getDouble("rating");
                    %>
                    <option value="<%= therapistId %>"><%= therapistName %><%= specialization != null ? " - " + specialization : "" %> (Rating: <%= String.format("%.1f", rating) %>)</option>
                    <%
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
            ...
            <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-200 px-4 py-3 rounded-lg">
                <p class="text-sm"><span class="material-icons text-sm align-middle mr-1">info</span> Your booking will be pending until approved by the salon owner.</p>
            </div>

            <div class="flex justify-end space-x-4">
                <a href="salon-details.jsp?id=<%= salonId %>" class="px-6 py-3 border border-gray-300 dark:border-gray-600 rounded-full font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                    Cancel
                </a>
